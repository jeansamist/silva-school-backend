  ...
  def post(self, request, class_level_pk):
    ...
    try:
      ...
      name = "{classname}E{class_id}".format(
          classname=class_level.name, class_id=ClassRoom.objects.filter(class_level=class_level_pk).count() + 1)
    try:
      user = User.objects.get(pk=request.current_user.pk)
    except:
      return Response({'details': 'You cannot access to a ClassLevel. Try to login'}, HTTP_401_UNAUTHORIZED)
...
class ClassLevelView(APIView):
  def get(self, request):
    ...
    try:
      ...
      return Response({'details': 'You cannot access to a ClassLevel. give a School id'}, HTTP_401_UNAUTHORIZED)
    ...
    if user.role.name == Role.objects.get(name='root').name:
      ...
      if user.schools.filter(pk=school_id).count() == 1:
        ...
        return Response({'details': 'You cannot access to this School id'}, HTTP_401_UNAUTHORIZED)
